...
namespace yolov5
{
    ...
    int loadLabelName(std::string locationFilename, char *label[],int class_num)
    {
        printf("loadLabelName %s\n", locationFilename.c_str());
        readLines(locationFilename.c_str(), label, class_num);
        // 将label打印一遍
    ...
    post_process(int8_t *input0, int8_t *input1, int8_t *input2, int model_in_h, int model_in_w, float conf_threshold,
                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group,std::string model_labels_path,int class_num)
    {
        if (init == -1)
        {
            int ret = 0;
            labels = new char*[class_num];
            ret = loadLabelName(model_labels_path, labels, class_num);
            //打印labels中的数据
            for (int i = 0; i < class_num; ++i)
            {
                printf("labels[%d]=%s\n", i, labels[i]);
